...
function SignInForm({ locale }: { locale: Locale }) {
  const t = getT(locale);
  ...
  async function submitLogin(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) throw new Error(data.error || t('adminAuth.signIn.errorLogin'));
      window.location.replace('/admin');
    } catch (err: unknown) {
      setError(err instanceof Error ? err.message : t('adminAuth.signIn.errorGeneric'));
    } finally {
      setLoading(false);
    }
  }
  ...
  async function submitForgot(e: React.FormEvent) {
    ...
    try {
      ...
      const data = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error(data.error || t('adminAuth.signIn.errorGeneric'));
      setForgotSent(true);
    } catch (err: unknown) {
      setError(err instanceof Error ? err.message : t('adminAuth.signIn.errorGeneric'));
    } finally {
    ...
    <div style={{ width: '100%', maxWidth: 400 }}>
      {!showForgot ? (
        <>
          ...
          <form onSubmit={submitLogin} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            <div>
              ...
              <input
                ...
                required
                autoComplete="email"
                placeholder={t('adminAuth.signIn.emailPlaceholder')}
                style={inputStyle}
              />
            </div>
            ...
            <div>
              <label style={{ display: 'block', fontSize: 13, fontWeight: 700, marginBottom: 6 }}>
                {t('adminAuth.signIn.passwordLabel')}
              </label>
              <div style={{ position: 'relative' }}>
                <input
                  ...
                  placeholder={t('adminAuth.signIn.passwordPlaceholder')}
                  style={{ ...inputStyle, paddingRight: 42 }}
                />
                <button
                  type="button"
                  onClick={() => setShowPassword(v => !v)}
                  aria-label={showPassword ? t('adminAuth.signIn.hidePassword') : t('adminAuth.signIn.showPassword')}
            ...
            <button type="submit" disabled={loading} style={btnStyle}>
              {loading ? t('adminAuth.signIn.submitting') : t('adminAuth.signIn.submit')}
            </button>
          </form>
          ...
          >
            {t('adminAuth.signIn.forgot')}
          </button>
        </>
      ) : forgotSent ? (
        <div style={{ textAlign: 'center' }}>
          <p style={{ fontSize: 18, fontWeight: 700, margin: '0 0 10px' }}>{t('adminAuth.forgot.sentTitle')}</p>
          <p style={{ color: 'rgba(0,0,0,0.5)', lineHeight: 1.6, fontSize: 14 }}>
            {t('adminAuth.forgot.sentBody')}
          </p>
          <button
            type="button"
          ...
          >
            {t('adminAuth.forgot.backToSignIn')}
          </button>
        </div>
      ) : (
        <>
          ...
          >
            {t('adminAuth.forgot.back')}
          </button>

          <h1 style={{ margin: '0 0 6px', fontSize: 22, fontWeight: 900, letterSpacing: '-0.03em' }}>
            {t('adminAuth.forgot.title')}
          </h1>
          <p style={{ margin: '0 0 24px', color: 'rgba(0,0,0,0.45)', fontSize: 14, lineHeight: 1.5 }}>
            {t('adminAuth.forgot.subtitle')}
          </p>
          ...
          <form onSubmit={submitForgot} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <button type="submit" disabled={loading} style={btnStyle}>
              {loading ? t('adminAuth.forgot.submitting') : t('adminAuth.forgot.submit')}
            </button>
          </form>
        </>
      )}
