...
${slotsText ? `СВОБОДНИ ЧАСОВЕ ПО МАЙСТОР (следващите 7 дни):\n${slotsText}` : ''}` : '';
  ...
  const bookingLinkRule = isTeamPlan
    ...
    : 'Когато клиент пита за цена или иска да запише конкретна услуга, добави в КРАЯ на отговора си (на нов ред) тага <<BOOK_LINK:ТОЧНО_ИМЕ_НА_УСЛУГАТА>> с точното име на услугата от списъка. Примери: <<BOOK_LINK:Маникюр>>, <<BOOK_LINK:Педикюр с лак>>. Не добавяй тага ако клиентът не пита за конкретна услуга.';
