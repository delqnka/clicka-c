...
export interface CreatePageConfig {
  /** Show the "6 месеца" period toggle. Default: true */
  showSixMonth?: boolean;
  /** Where the "← Назад" link points. Default: "/" */
  backHref?: string;
}
...
function resolveInitial(param: string | null, showSixMonth: boolean): { plan: Plan; period: Period } {
  const period: Period = showSixMonth && param === 'solo_6m' || param === 'team_6m' ? '6m' : '12m';
  if (param === 'team_12m' || param === 'team_6m' || param === 'ekip' || param === 'team') {
    return { plan: 'team', period };
  }
  return { plan: 'solo', period };
}

// ─── Main component ───────────────────────────────────────────────────────────
export function CreatePageContent({ showSixMonth = true, backHref = '/' }: CreatePageConfig) {
  const searchParams = useSearchParams();
  const grantToken   = searchParams.get('grant');
  const promoRef     = searchParams.get('ref') ?? undefined;
  const initial      = resolveInitial(searchParams.get('plan'), showSixMonth);

  const [plan,   setPlan]   = useState<Plan>(initial.plan);
  // When showSixMonth=false we always force 12m, regardless of URL param
  const [period, setPeriod] = useState<Period>(showSixMonth ? initial.period : '12m');
  ...
  // Restore draft on back-navigation from Stripe
  useEffect(() => {
    try {
  ...
  async function handlePay() {
    ...
    if (grantToken) {
      if (!ownerName.trim()) { setError('Въведи твоето ime.'); return; }
      if (!salonName.trim()) { setError('Въведи ime на салона.'); return; }
    ...
    try {
      const res  = await fetch('/api/create-checkout', {
        ...
        body: JSON.stringify({
          ...
          slug:       slug.trim(),
          ...(promoRef && { promoRef }),
          ...(wantsInvoice && { billingInfo: { companyName: companyName.trim(), eik: eik.trim(), vatNumber: vatNumber.trim() || null } }),
        }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || 'Грешка');
  ...
  return (
    ...
    >
      ...
      <nav
        ...
        style={{ background: 'rgba(255,255,255,0.93)', backdropFilter: 'blur(24px) saturate(180%)', WebkitBackdropFilter: 'blur(24px) saturate(180%)' }}
      >
        <ClickaLogo size="nav" />
        <a href={backHref} className="text-[13px] font-semibold text-[#6b7280] hover:text-[#0a0a0a] transition-colors">
          ← Назад
        </a>
      </nav>
      ...
      <div className="mx-auto max-w-[600px] px-4 pb-6 pt-8 sm:px-5 sm:pb-24 sm:pt-12">
        ...
        <p className="mb-8 text-[15px] text-[#6b7280]">Еднократно плащане. Без месечни такси.</p>
        ...
        <div className="mb-6 flex gap-3">
          {showSixMonth ? (
            ...
            <button type="button" className="cp-period-btn selected" style={{ cursor: 'default' }}>
              Годишно
              <span className="cp-period-badge">Най-изгодно</span>
            </button>
          )}
        </div>
        ...
        <div className="flex flex-col gap-4">
          ...
          >
            <div className="flex items-start justify-between gap-4">
              ...
              <div style={{ textAlign: 'right', flexShrink: 0 }}>
                <p style={{ fontSize: 26, fontWeight: 800, letterSpacing: '-0.04em', margin: 0, fontVariantNumeric: 'tabular-nums' }}>{formatDualEurParts(PRICES.solo[period]).eur}</p>
                <p style={{ fontSize: 10, color: '#9ca3af', margin: '1px 0 0', fontVariantNumeric: 'tabular-nums' }}>{formatDualEurParts(PRICES.solo[period]).bgn}</p>
                <p style={{ fontSize: 11, color: '#6b7280', margin: '2px 0 0' }}>{period === '12m' ? '/ година' : '/ 6 месеца'}</p>
              </div>
            </div>
            {plan === 'solo' && (
              <>
          ...
          >
            <div className="flex items-start justify-between gap-4">
              ...
              <div style={{ textAlign: 'right', flexShrink: 0 }}>
                <p style={{ fontSize: 26, fontWeight: 800, letterSpacing: '-0.04em', margin: 0, fontVariantNumeric: 'tabular-nums' }}>{formatDualEurParts(PRICES.team[period]).eur}</p>
                <p style={{ fontSize: 10, color: '#9ca3af', margin: '1px 0 0', fontVariantNumeric: 'tabular-nums' }}>{formatDualEurParts(PRICES.team[period]).bgn}</p>
                <p style={{ fontSize: 11, color: '#6b7280', margin: '2px 0 0' }}>{period === '12m' ? '/ година' : '/ 6 месеца'}</p>
              </div>
            </div>
            {plan === 'team' && (
        ...
        <div className="mt-6">
          ...
          {emailStatus === 'exists' && (
            <p className="mt-1 text-[12px] font-semibold text-red-600">Имейлът вече съществува в системата, моля използвайте друг.</p>
          )}
        </div>
        ...
        <div className="mt-6">
          ...
          {slugStatus === 'taken'     && <p className="mt-1 text-[12px] font-semibold text-red-600">Този адрес вече е зает — избери друг</p>}
        </div>
        ...
        <div className="mt-4">
          <label className="flex cursor-pointer items-center gap-3">
            ...
            <span className="text-[13px] font-semibold text-[#0a0a0a]" onClick={() => setWantsInvoice(v => !v)}>Искам фактура на фирма</span>
          </label>
          {wantsInvoice && (
